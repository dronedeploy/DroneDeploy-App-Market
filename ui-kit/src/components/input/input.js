(function() {
  ...
  function build(document) {
    ...
    const inputPrototype = Object.create(HTMLElement.prototype)
    inputPrototype.createdCallback = function() {
      ...
      <style>
        ...
        input[type="text"].dd-input:focus,
        input[type="text"].dd-input.focused {
          outline: none;
        ...
        label {
          ...
          font-size: 10.5px; /*Should be 14px, scaled down in animation */
        ...
        <form>
          <label>${label}</label>
          <br />
          <input type="text" value="${defaultValue}" class="dd-input" tabindex="0"/>
        </form>
      `
    }

    inputPrototype.attachedCallback = function() {
      const ddInput = this.root_.querySelector('.dd-input')
      const ddLabel = this.root_.querySelector('label')
      ddInput.onfocus = function() {
        this.classList.add('focused')
        ddLabel.classList.add('focused')
      }

      ddInput.onblur = function() {
        this.classList.remove('focused')
        ddLabel.classList.remove('focused')
      }

      ddInput.onchange = event => {
        this.value = event.target.value
        this.dispatchEvent(new CustomEvent('change'))
      }
    }

    document.registerElement('dd-input', { prototype: inputPrototype })
  }
})()