(function(document) {
  ...
  function build(document) {
    ...
    const radioPrototype = Object.create(HTMLElement.prototype)
    radioPrototype.createdCallback = function() {
      ...
      const values = this.innerHTML.split(',')
        .map(value => value.replace(' ',''))
        .reduce((accumulator, currentValue) => accumulator.indexOf(currentValue) > -1 ? accumulator : [...accumulator, currentValue], [])
        .map((value, i) =>`
              <div class="radio" tabindex="0">
                <div class="radio-btn ${selected === value ? 'active' : ''}"><i class="icon icon-radio-button-checked"></i><i class="icon icon-radio-unchecked"></i></div>
      ...
      <style>
        ...
        .active .icon-radio-unchecked {
          display: none;
        }

        .active .icon-radio-button-checked {
          display: inline-block;
        }
    ...
    radioPrototype.attachedCallback = function() {
      const btns = Array.from(this.root_.querySelectorAll('.radio'))
      btns.forEach(btn => btn.onclick = event => {
        btns.forEach(btn => removeActive(btn))
        setActive(this, event.target, btn)
        setTimeout(() => { btn.blur() }, 200)
      })

      btns.forEach(btn => btn.onkeypress = event => {
      ...
      function removeActive(btn) {
        btn.querySelector('.radio-btn').classList.remove('active')
      }

      function setActive(self, target, btn) {
        while(target !== btn) {
            target = target.parentNode
        }
        target.focus
        target.querySelector('.radio-btn').classList.add('active')
        self.value = btn.querySelector('.label').innerHTML
        self.dispatchEvent(new CustomEvent('change'))
      }
    }


    document.registerElement('dd-radio', { prototype: radioPrototype })
  }
})(document)