(function() {
  ...
  function build(document) {
    ...
    infoPrototype.createdCallback = function() {
      ...
      if(!document.querySelector('head').classList.contains('dd-font')) {
        const documentHead = document.head
      ...
      this.root_.innerHTML = `
        <style>
            ...
            .gotit {
              margin: 0;
              margin-left: ${readMoreLink ? '0' : 'auto'};
              margin-right: 0;
              display: inline-flex;
              border: none;
              color: #2196F3;
          ...
          <i class="icon icon-question-circle" id="toggle-infobox" tabindex="0"></i>
          <aside class="infobox" id="info-popup">
            <header class="title">
              ${title}
            </header>
            ...
            <div class="links">
              ${readMoreLink ? `<a href="${readMoreLink}" target="_blank" class="readmore link">READ MORE</a>` : ''}
              <button type="button" class="gotit link" id="hide-button">GOT IT</button>
            </div>
    ...
    infoPrototype.attachedCallback = function() {
      function toggleActive(element) {
        const _e = element
        return function(event) {
          if(_e.offsetWidth + event.clientX >= window.innerWidth) {
            _e.style.left = (window.innerWidth - _e.offsetWidth - 10) + 'px'
          } else {
            _e.style.left = (event.clientX + 10) + 'px'
          }
          if(_e.offsetHeight + event.clientY >= window.innerHeight) {
            _e.style.top = (window.innerHeight - _e.offsetHeight - 10) + 'px'
          } else {
            _e.style.top = (event.clientY - _e.offsetHeight - 10) + 'px'
          }
          _e.classList.toggle('active')
        }
      }
      const hideInfoButton = this.root_.getElementById('hide-button')
      const infoPopup = this.root_.getElementById('info-popup')
      const toggleInfobox = this.root_.getElementById('toggle-infobox')
      const overlay = this.root_.querySelector('.overlay')
      hideInfoButton.onclick = toggleActive(infoPopup)
      toggleInfobox.onclick = toggleActive(infoPopup)
      overlay.onclick = event => infoPopup.classList.remove('active')
      toggleInfobox.onkeypress = event => {
        if(event.keyCode === 32) { toggleActive(infoPopup)(event) }
      }
    }


    document.registerElement('dd-info', { prototype: infoPrototype })
  }
})()