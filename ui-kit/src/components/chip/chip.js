(function() {
  ...
  function build(document) {
    ...
    const chipPrototype = Object.create(HTMLElement.prototype)
    chipPrototype.createdCallback = function() {
      ...
      this.root_.innerHTML = `
        <style>
          ...
          .icon-close-mdi:before {
              content: '\\f02f';
          }
        </style>
        <div class="chip">
          <slot></slot>
          <span id="remove-button"><i class="icon icon-close-mdi"></i></span>
        </div>
      `
    }

    chipPrototype.attachedCallback = function() {
      const self = this
      this.root_.getElementById('remove-button').onclick = function() {
        self.parentNode.removeChild(self)
      }
    }

    document.registerElement('dd-chip', { prototype: chipPrototype })
  }
})()